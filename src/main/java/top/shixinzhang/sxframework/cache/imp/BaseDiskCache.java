...
public class BaseDiskCache implements IDiskCache {
     ...
     */
    public BaseDiskCache(File rootDirectory, int maxCacheSizeInBytes) {
    ...
    public synchronized Entry get(@NonNull String key) {
        ...
        try {
            ...
            LogUtils.d("%s: %s", file.getAbsolutePath(), e.toString());
            remove(key);
            return null;
    ...
    public void initialize() {
        if (!mRootDirectory.exists()) {
            if (!mRootDirectory.mkdirs()) {
                LogUtils.e("Unable to create cache dir %s", mRootDirectory.getAbsolutePath());
            }
            return;
        }
        ...
        for (File file : files) {
            ...
            try {
                ...
                synchronized (BaseDiskCache.this) {
    ...
    public synchronized void put(@NonNull String key, @NonNull Entry entry) {
        ...
        try {
            ...
            if (!success) {
                fos.close();
                LogUtils.d("Failed to write header for %s", file.getAbsolutePath());
        ...
        if (!deleted) {
            LogUtils.d("Could not clean up file %s", file.getAbsolutePath());
        }
    }
    ...
    public synchronized void remove(@NonNull String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            LogUtils.d("Could not delete cache entry for key=%s, filename=%s",
    ...
    private void pruneIfNeeded(int neededSpace) {
        ...
        if (LogUtils.isDebug()) {
            LogUtils.v("Pruning old cache entries.");
        }
        ...
        if (LogUtils.isDebug()) {
            LogUtils.v("pruned %d files, %d bytes, %d ms",
